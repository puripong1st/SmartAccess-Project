# 📘 คู่มือระบบควบคุมประตูอัจฉริยะ (SmartAccess Door Access System Manual)
...
ระบบ **SmartAccess Door Access** ได้รับการพัฒนาขึ้นภายใต้แนวคิด **"Single Centralized Server, Distributed Edge Nodes"** เพื่อแก้ปัญหาข้อจำกัดเดิมที่ต้องติดตั้งเซิร์ฟเวอร์แยกทีละห้องเรียน ปัจจุบันเซิร์ฟเวอร์เพียงเครื่องเดียวสามารถควบคุมและส่งผ่านสัญญาณเปิดประตูไปยังบอร์ด ESP32 ได้อย่างอิสระไร้ขีดจำกัด (>2 ห้องขึ้นไป)
...
graph TD
    ...
    subgraph Edge_Classrooms ["Edge Classrooms (Distributed IoT)"]
        direction TB
        subgraph CE_401 ["Classroom CE-401"]
        ...
        subgraph CE_402 ["Classroom CE-402"]
...
* **Database Driver:** Connection Pool ของ `pg/promise` ป้องกันการเชื่อมต่อล้น
...
#### 1) ระบบปลดล็อกทันทีแบบเรียลไทม์ (Real-Time MQTT Push - Subscription):
* **การเชื่อมต่อ:** บอร์ด ESP32 จะทำการเชื่อมต่อกับโบรกเกอร์กลาง **HiveMQ Cloud** ผ่านเครือข่ายอินเทอร์เน็ตแบบเข้ารหัส TLS (Secure TCP พอร์ต 8883) ตลอดเวลา
* **การติดตามคำสั่ง (Subscription):** บอร์ดสมัครรับข้อมูล (Subscribe) หัวข้อคำสั่งส่วนบุคคลตามห้อง เช่น:
  `smartaccess/rooms/<ROOM_CODE>/command`
* **การปลดล็อกด่วน (<100ms):** เมื่อแอดมินกดอนุมัติ ระบบ Next.js เซิร์ฟเวอร์จะทำการส่งข้อความ JSON (เช่น `{"action":"unlock", "name":"...", "student_id":"..."}`) ผ่าน MQTT WebSockets (พอร์ต 8884) ไปยังโบรกเกอร์ ซึ่งจะส่งต่อไปยัง ESP32 ทันที บอร์ดจะกระตุ้นรีเลย์เปิดประตูทันทีโดยไม่มีดีเลย์ของ HTTP Polling
...
#### 3) ขาเปิดเซิร์ฟเวอร์สำรองใน LAN (Local Web Server - Port 80):
* บอร์ดเปิดเว็บเซิร์ฟเวอร์ LAN ภายในตัวเอง รอรับคำสั่งทริกฉุกเฉินกรณีการเชื่อมต่อระบบคลาวด์มีปัญหา
* **Offline PIN & QR Cache:** หากบอร์ดขาดการเชื่อมต่ออินเทอร์เน็ตและสลับเข้าสู่โหมดออฟไลน์ ผู้ใช้สามารถป้อนรหัส PIN หรือสแกนคีย์ QR โค้ดที่ออกไว้ล่วงหน้าเพื่อปลดล็อกได้โดยบอร์ดจะวิเคราะห์คีย์ผ่านลายเซ็นดิจิทัล HMAC-SHA256 ที่บันทึกไว้ในแคชระบบไฟล์ SPIFFS ของตัวเองโดยไม่ต้องพึ่งพาอินเทอร์เน็ต
* **ความปลอดภัย:** ป้องกันการยิงไอพีเปิดประตูตรงด้วย Zero-Trust Headers โดยเช็ก Timestamp, Nonce, API Key ร่วมกับ Signature HMAC-SHA256 ทุกครั้ง
