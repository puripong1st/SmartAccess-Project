...
<body>
  ...
  <script>
    ...
    window.saveCodeAsPng = function(codeWindowElement, fileName) {
      ...
      // Clone the code-window into an off-screen container with ALL overflow/height
      // constraints removed so html2canvas captures the FULL code block content.
      var clone = codeWindowElement.cloneNode(true);

      // Strip save buttons from clone
      clone.querySelectorAll(".code-save-btn").forEach(function(btn) {
        btn.parentNode.removeChild(btn);
      });

      // Remove overflow clipping on the code-window itself
      clone.style.overflow = "visible";
      clone.style.maxHeight = "none";
      clone.style.height = "auto";

      // Remove overflow clipping on pre/code inside
      ...
      // Create an off-screen wrapper that html2canvas can render at full size
      var wrapper = document.createElement("div");
      wrapper.style.position = "absolute";
      wrapper.style.top = "0";
      wrapper.style.left = "0";
      wrapper.style.width = Math.max(codeWindowElement.scrollWidth, codeWindowElement.offsetWidth, 800) + "px";
      ...
      // Small delay to let the browser lay out the clone fully
      setTimeout(function() {
        html2canvas(clone, {
          scale: 2,
          useCORS: true,
      ...
      window.exportSection = function(headerId, format) {
        ...
        const canvasOptions = {
          ...
          onclone: function(clonedDoc) {
            ...
            // Remove overflow:hidden from code-window containers so content is never clipped
            clonedDoc.querySelectorAll(".code-window").forEach(el => {
              el.style.overflow = "visible";
              el.style.maxHeight = "none";
              el.style.height = "auto";
            });
